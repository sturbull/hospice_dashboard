...
box::use(
  app / logic / ReturnValueFunctions[GetHospiceUser,GetPatientCounts]
)
#' @export
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$PatientsValue <- renderUI({

      metrics_patients <- GetPatientCounts(HOSPICE())

      return(paste0("There were ",metrics_patients," patients on the books."))
    })
  })
}
