box::use(
  shiny[div, tags, moduleServer, NS, HTML],
...
ui <- function(id) {
  ...
  div(
    id = id,
    class = "text-center flex flex-col items-center justify-center py-16 px-4 sm:px-6 lg:px-8",
    empty_box_svg(class = "h-48 w-48 lg:h-64 lg:w-64 text-primary-600 dark:text-primary-500"),
    # tags$p("404", class = "text-base font-semibold text-primary-600 dark:text-primary-400"),
    tags$h1("Page not found", class = "mt-8 text-3xl font-bold tracking-tight text-slate-700 dark:text-slate-200 sm:text-5xl",`data-animate`="true", style="--start:200ms"),
    tags$p("Sorry, we couldn’t find the page you’re looking for.", class = "mt-4 text-lg text-slate-500 dark:text-slate-400",`data-animate`="true", style="--start:400ms"),
    tags$div(
      class = "mt-8 flex justify-center",
      `data-animate`="true", style="--start:600ms",
      tags$a(
        "Go back home",
        href = route_link("/"),
