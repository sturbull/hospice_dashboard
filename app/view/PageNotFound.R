...
empty_box_svg <- function(class) {
  HTML(
    paste0("<svg clip-rule='evenodd' fill-rule='evenodd' stroke-linejoin='round' stroke-miterlimit='2' viewBox='0 0 64 64'",
           "class='", class, "'",
...
ui <- function(id) {
  ...
  div(
    ...
    tags$h1("Page not found", class = "mt-8 text-3xl font-bold tracking-tight text-slate-700 dark:text-slate-200 sm:text-5xl"),
    tags$p("Sorry, we couldn’t find the page you’re looking for.", class = "mt-4 text-lg text-slate-500 dark:text-slate-400"),
    tags$div(
